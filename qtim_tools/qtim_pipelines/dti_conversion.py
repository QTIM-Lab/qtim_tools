...
def qtim_dti_conversion(
    ...
    studies = glob.glob(
        os.path.join(base_directory, study_name, "RAWDATA", study_name + "*/")
    )
    visits = []
    for study in studies:
        visits += glob.glob(os.path.join(study, "*/"))
    ...
    for volume in original_volumes:
        ...
        if specific_case is not None and (
            patient_name + "-" + visit_name != specific_case
        ):
            continue
        ...
        # Convert from DICOM
        output_file_prefix = (
            os.path.join(output_folder, patient_name + "-" + visit_name) + "-DTI_diff"
        )
        ...
        bval, bvec, diff = [
            output_file_prefix + ".bval",
            output_file_prefix + ".bvec",
            output_file_prefix + ".nii.gz",
        ]
        if overwrite or not (
            os.path.exists(output_file_prefix + ".bval")
            and os.path.exists(output_file_prefix + ".bvec")
            and os.path.exists(output_file_prefix + ".nii.gz")
        ):
            # print(convert_DTI_nifti(volume, output_folder, output_file_prefix))
            # bval, bvec, diff =
            convert_DTI_nifti(volume, output_folder, output_file_prefix)
        ...
        # Rotate bvecs
        output_rotated_bvec_file = (
            os.path.splitext(output_bvec_file)[0]
            + "_rotated"
            + os.path.splitext(output_bvec_file)[-1]
        )
        input_motion_file = nifti_splitext(diff)[0] + "_mc.ecclog"
        if overwrite or not os.path.exists(output_rotated_bvec_file):
            run_fdt_rotate_bvecs(
                output_bvec_file, output_rotated_bvec_file, input_motion_file
            )

        # Run Skull-Stripping
        skull_strip_mask = (
            nifti_splitext(output_motion_file)[0]
            + "_ss_mask"
            + nifti_splitext(output_motion_file)[-1]
        )
        skull_strip_output = (
            nifti_splitext(output_motion_file)[0]
            + "_ss"
        ...
        if overwrite or not os.path.exists(skull_strip_mask):
            skull_strip(
                ...
                skull_strip_output,
                skull_strip_mask,
                extra_parameters={"fsl_threshold": 0.1},
            )

        # Fit DTI
        output_prefix = os.path.join(
            output_folder, patient_name + "-" + visit_name + "-DTI"
        )
...
def run_fdt_rotate_bvecs(input_bvec, output_bvec, input_motion):
    script_location = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "external", "fdt_rotate_bvecs.sh")
    )
...
def run_1dtool(input_bvec, output_bvec):
    script_location = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "external", "1d_tool.py")
    )
    ...
    subprocess.call(
        [script_location, "-infile", input_bvec, "-transpose", "-write", output_bvec]
    )
...
def convert_DTI_nifti(volume, output_folder, output_file_prefix):
    ...
    if DTI:
        subprocess.call(
            # ["dcm2nii", "-d", "N", "-i", "N", "-p", "N", "-o", output_folder, volume]
            ["dcm2niix", "-o", output_folder, volume]
        )
    ...
    for output_file in output_files:
        renamed_file = os.path.join(
            ...
            output_file_prefix
            + "."
            + ".".join(str.split(os.path.basename(output_file), ".")[1:]),
        )
...
def check_DTI(sequence, protocol, bval):
    ...
    if not any(
        [
            x in ["tof", "fl3d", "memp", "fse", "grass", "3-Plane", "gre"]
            for x in sequence
        ]
    ):

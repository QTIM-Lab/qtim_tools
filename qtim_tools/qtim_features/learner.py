...
def generate_feature_list_batch(folder, features=['GLCM', 'morphology', 'statistics'], labels=False, label_suffix="-label", decisions=False, levels=255, filenames=True, featurenames=True, outfile='', overwrite=True, write_empty=True, mask_value=0, test=False, use_labels=[-1], erode=0):
    ...
    # It also makes sense to clear file contents at the beginning of each program run. (TO-DO)
    ...
    if outfile != '':
        ...
        with open(outfile, 'ab') as writefile:
            ...
            imagepaths, label_images = generate_filename_list(folder, labels, label_suffix)
            ...
            for imagepath in imagepaths:
                ...
                image_list, imagename_list, attributes_list = generate_numpy_images(imagepath, labels=labels, label_suffix=label_suffix, label_images=label_images, levels=levels, mask_value=mask_value, use_labels=use_labels, erode=erode)
                ...
                for image_idx, image in enumerate(image_list):
                    ...
                    if numerical_output[0,0] == 0:
                        numerical_output[0, :] = generate_feature_list_method(image, attributes_list[image_idx], features, feature_indexes, total_features, levels, mask_value=0)
                        index_output[0,:] = index
                    else:
                        numerical_output = np.vstack((numerical_output, generate_feature_list_method(image, attributes_list[image_idx], features, feature_indexes, total_features, levels, mask_value=0)))
...
def generate_feature_list_single(vol_filename, features=['GLCM', 'morphology', 'statistics'], labels=False, label_filename='',label_suffix="-label", decisions=False, levels=255, filenames=True, featurenames=True, outfile='', overwrite=True, write_empty=True, mask_value=0, test=False, use_labels=[-1], erode=0):
    ...
    if outfile != '':
        ...
        with open(outfile, 'ab') as writefile:
            ...
            image_list, imagename_list, attributes_list = generate_numpy_images(vol_filename, labels=labels, label_suffix=label_suffix, label_images=label_filename, levels=levels, mask_value=mask_value, use_labels=use_labels, erode=erode)
            
            if image_list == []:
                if write_empty:
                    empty_output = np.zeros((1, total_features + 1), dtype=object)
                    empty_output[0,0] = vol_filename
...
def generate_filename_list(folder, labels=False, label_suffix='-label'):
...
def generate_numpy_images(imagepath, labels=False, label_suffix='-label', label_images=[], mask_value=0, levels=255, use_labels=[-1], erode=0):
    ...
    return [image_list, imagename_list, attributes_list]

def generate_feature_list_method(image, attributes, features, feature_indexes='', total_features='', levels=-1, mask_value=0):
    ...
    for feature_idx, feature in enumerate(features):
        ...
        if feature == 'morphology':
            ...
            numerical_output[0, feature_indexes[feature_idx]:feature_indexes[feature_idx+1]] = morphology.morphology_features(image, attributes)
